...
const loginUser = async (req, res) => {
	...
	try {
		...
		// Return the user details without JWT token
		res.status(200).json({ 
			success: true,
...
const submitFeedback = async (req, res) => {
	try {
		...
		const { serviceId, feedback, rating, userId } = req.body;

		if (!serviceId || !feedback || !rating || !userId) {
			return res.status(400).json({ message: "Missing required fields (serviceId, feedback, rating, userId)" });
		}
